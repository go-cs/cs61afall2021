...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0 and self.balance == 0:
            ...
            self.stock -= 1
            self.balance = 0#找零钱后要清零
            return f'Here is your {self.product} and ${change} change.'
    def add_funds(self, money):
        self.balance += money
        if self.stock == 0:
            now_balance = self.balance
            self.balance = 0#退钱了，此处也得清零
            return f'Nothing left to vend. Please restock. Here is your ${now_balance}.'
        else:
            return f'Current balance: ${self.balance}'
    def restock(self, store):
        self.stock += store
