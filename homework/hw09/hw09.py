...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r"[CScs]+\s*\d+[ABCabc]?", post))
    #return re.findall(r"[CScs]+\s*\d+[ABCabc]?", post)
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r"[0123]?[0-9][:][0-5][0-9][AM]*", text)
...
def most_common_code(text):
    ...
    #     max = code.count(code[0])
    #     #print('DEBUG:', max, code[i])
    #     if code.count(code[i]) >= max:
    #         mc = code[i]
    # return mc
