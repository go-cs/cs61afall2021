...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        #你的新框架应该是定义 lambda 的框架的子框架。
        frame = procedure.env.make_child_frame(procedure.formals, args)#必须是procedure.env而不是直接用env
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        ...
        expr = expressions
        while expr.rest != nil:
            scheme_eval(expr.first, env)#需要在当前环境中加上expressions前边式子的执行结果
            #比如scm> (begin (define x 3) x)需要执行(define x 3)
            expr = expr.rest
