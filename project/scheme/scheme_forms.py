...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(symbol, value)#env是Frame实例expressions是Pair实例
        return symbol#scheme定义完后输出的是一个符号
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        # https://github.com/Sa1tyee/CS61A-Fall2021/blob/main/scheme/scheme_forms.py
        symbol = signature.first
        lambda_expressions = Pair(signature.rest, expressions.rest)
        procedure = do_lambda_form(lambda_expressions, env)#生成一个LambdaProcedure
...
def do_lambda_form(expressions, env):
    ...
    expr = LambdaProcedure(formals, body, env)
    return expr
    # END PROBLEM 7
...
def do_if_form(expressions, env):
    ...
    if is_scheme_true(scheme_eval(expressions.first, env)):#判断是#t还是#f，#t执行第二个
        return scheme_eval(expressions.rest.first, env)
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env)# #f执行第三个表达式
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            #test==True有两种情况：1.第一个表达式的第一个符号为else 2.第一个表达式为#t
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            #expressions.first是圆括号括起来的内容
            #clause.first是第一个表达式的
            # if clause.first == 'else' and clause.rest == nil:
            #     return True
            # elif clause == True and clause.rest == nil:
            #     return scheme_eval(clause.first, env)

            #用下边两行代替上边4行，还有elif下的clause.first==True不一定成立啊
            if clause.rest == nil:
                return test
